...
import type { SavedDispensa, SavedSimulation, SimulationQuestion, SubjectDetails, ExamStructure, LearnedExam, EsercizioTopic, ChatMessage } from '../types';
...
interface SimulazioniEsamiViewProps {
    ...
    onStartOralExam: (simulation: SavedSimulation) => Promise<string>;
    onContinueOralExam: (simulation: SavedSimulation, history: ChatMessage[]) => Promise<string>;
    onFinishOralExam: (simulation: SavedSimulation, history: ChatMessage[]) => Promise<void>;
...
export const SimulazioniEsamiView: React.FC<SimulazioniEsamiViewProps> = ({ selectedSubject, dispense, simulations, subjectDetails, onGenerate, onSave, onLearnExamStructure, onStartOralExam, onContinueOralExam, onFinishOralExam, isGenerating, error, clearError, t, dataVersion }) => {
    type ViewState = 'home' | 'create' | 'upload' | 'list' | 'taking' | 'results' | 'listTracce' | 'detailTraccia' | 'oral';
    ...
    const [oralTranscript, setOralTranscript] = useState<ChatMessage[]>([]);
    const [oralInput, setOralInput] = useState('');
    const [isOralProcessing, setIsOralProcessing] = useState(false);
    const oralMessagesEndRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        oralMessagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }, [oralTranscript]);
    ...
    const handleStructureChange = (field: keyof ExamStructure, value: any) => {
        if (field === 'hasWritten' || field === 'hasOral') {
            setExamStructure(prev => ({ ...prev, [field]: value }));
            return;
        }
    ...
    const handleStartOral = async () => {
        const sim = reviewingSimulation || activeSimulation;
        if (!sim) return;

        setIsOralProcessing(true);
        const openingMessage = await onStartOralExam(sim);
        setOralTranscript([{ id: Date.now().toString(), role: 'model', text: openingMessage }]);
        setActiveSimulation(sim); // Make sure this is the active sim now
        setView('oral');
        setIsOralProcessing(false);
    };

    const handleSendOralMessage = async () => {
        if (!oralInput.trim() || isOralProcessing || !activeSimulation) return;
        const userMessage: ChatMessage = { id: Date.now().toString(), role: 'user', text: oralInput };
        const newHistory = [...oralTranscript, userMessage];
        setOralTranscript(newHistory);
        setOralInput('');
        setIsOralProcessing(true);

        const responseText = await onContinueOralExam(activeSimulation, newHistory);
        const modelMessage: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: responseText };
        setOralTranscript(prev => [...prev, modelMessage]);
        setIsOralProcessing(false);
    };

    const handleFinishOral = async () => {
        if (!activeSimulation) return;
        if (!window.confirm("Sei sicuro di voler terminare l'esame orale? L'AI procederà alla valutazione finale.")) return;
        
        setIsOralProcessing(true);
        await onFinishOralExam(activeSimulation, oralTranscript);
        // The view will update via props, but we can optimistically switch
        setView('results');
        setIsOralProcessing(false);
    };
                        ...
                        <li key={sim.id} className="bg-gray-700 p-4 rounded-lg flex justify-between items-center">
                            ...
                            <div className="text-right">
                                <p className="font-bold text-lg text-white">
                                    {sim.finalGrade ? `Voto Finale: ${sim.finalGrade.toFixed(1)}/30` : (sim.status === 'oral_pending' ? `Scritto: ${sim.grade.toFixed(1)}/30` : `Voto: ${sim.grade.toFixed(1)}/30`)}
                                </p>
                    ...
                    <div className="space-y-4">
                        ...
                        <fieldset className="border border-gray-700 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-2">
                                <div className="col-span-2 lg:col-span-4 flex items-center gap-6">
                                    <label className="flex items-center space-x-2 cursor-pointer">
                                        <input type="checkbox" checked={examStructure.hasWritten} onChange={e => handleStructureChange('hasWritten', e.target.checked)} className="h-4 w-4 rounded bg-gray-900 border-gray-600 text-indigo-500 focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-700 focus:ring-indigo-500" />
                                        <span>Scritto</span>
                                    </label>
                                     <label className="flex items-center space-x-2 cursor-pointer">
                                        <input type="checkbox" checked={examStructure.hasOral} onChange={e => handleStructureChange('hasOral', e.target.checked)} className="h-4 w-4 rounded bg-gray-900 border-gray-600 text-indigo-500 focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-700 focus:ring-indigo-500" />
                                        <span>Orale</span>
                                    </label>
                                </div>
                                ...
                                <div className="col-span-2 lg:col-span-4 pt-3 mt-2 border-t border-gray-700/50 text-xs text-gray-400 bg-gray-900/50 p-3 rounded-lg border-l-2 border-indigo-500">
                        ...
                        <button
                            ...
                            disabled={showLoading || selectedDispenseIds.length === 0 || (!examStructure.hasWritten && !examStructure.hasOral) || (examStructure.hasWritten && totalQuestions === 0) }
                        ...
                        >
                            <PlusIcon />{t('simulations.startBtn')}
                        </button>
             ...
             <div className="bg-gray-800 rounded-xl p-6">
                <div className="flex justify-between items-start mb-4 pb-4 border-b border-gray-700">
                    <div>
                        ...
                        <p className="text-sm text-gray-400">Svolta il {new Date(simToReview.date).toLocaleString('it-IT')}</p>

                        <div className="flex items-baseline gap-4 mt-2">
                            {simToReview.finalGrade ? (
                                <>
                                    <p className="text-lg text-gray-300">Scritto: {simToReview.grade.toFixed(1)}/30</p>
                                    <p className="text-2xl font-bold text-indigo-400">Voto Finale: {simToReview.finalGrade.toFixed(1)} / 30</p>
                                </>
                            ) : (
                                <p className="text-2xl font-bold text-indigo-400">Voto Scritto: {simToReview.grade.toFixed(1)} / 30</p>
                            )}
                        </div>
                         {simToReview.status === 'oral_pending' && (
                            <button onClick={handleStartOral} disabled={isOralProcessing} className="mt-4 bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg flex items-center disabled:bg-gray-500">
                                {isOralProcessing ? 'Avvio...' : 'Inizia Esame Orale'}
                            </button>
                        )}
                    </div>
                ...
                {simToReview.oralEvaluation && (
                     <div className="bg-gray-900/50 p-4 rounded-lg mb-4">
                        <h3 className="text-lg font-bold text-white mb-2">Valutazione Esame Orale</h3>
                        <p className="text-gray-300 text-sm whitespace-pre-wrap">{simToReview.oralEvaluation}</p>
                    </div>
                )}
    ...
    const renderOralExamView = () => (
        <div className="bg-gray-800 rounded-xl p-6 flex flex-col w-full h-full">
            <header className="flex-shrink-0 p-4 bg-gray-900/50 flex justify-between items-center border-b border-gray-700">
                <div className="flex items-center gap-3">
                    <h2 className="text-xl font-bold text-white">Esame Orale: {selectedSubject}</h2>
                </div>
                <button onClick={handleFinishOral} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg">Termina Esame</button>
            </header>
            <div className="flex-1 p-4 overflow-y-auto">
                {oralTranscript.map(msg => (
                ...
                {isOralProcessing && (
                    <div className="flex items-start gap-3 my-4">
                        ...
                        <div className="bg-gray-700 rounded-lg p-3 max-w-md flex items-center">
                            ...
                            <span className="text-sm text-gray-300">L'esaminatore sta pensando...</span>
                        </div>
                    </div>
                )}
                <div ref={oralMessagesEndRef} />
            </div>
    ...
    if (view === 'taking' || view === 'oral') {
        return (
            <div className="fixed inset-0 bg-gray-900 z-[60] p-4 sm:p-8 flex flex-col">
                {view === 'taking' ? renderTakingView() : renderOralExamView()}
            </div>
        );
    }

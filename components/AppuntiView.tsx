...
import type { Appunto, SavedDispensa, Thumbnail, AiReviewResult } from '../types';
...
interface AppuntiViewProps {
    ...
    onGetAiReview: (note: Appunto, contextPages?: DbThumbnail[]) => Promise<AiReviewResult>;
    isProcessingAI: boolean;
    initialNoteToSelect: string | null;
    onNoteSelected: () => void;
    t: (key: string) => string;
    isEmbedded?: boolean;
}
...
export const AppuntiView: React.FC<AppuntiViewProps> = ({ subjectName, notes: initialNotes, dispense, allThumbnails, onSave, onDelete, onGetAiReview, isProcessingAI, t, isEmbedded = false, initialNoteToSelect, onNoteSelected }) => {
    const [notes, setNotes] = useState<Appunto[]>(initialNotes);
    ...
    useEffect(() => {
        if (initialNoteToSelect) {
            setActiveNoteId(initialNoteToSelect);
            onNoteSelected();
        }
    }, [initialNoteToSelect, onNoteSelected]);
    ...
    const buildNoteTree = useCallback((notesToTree: Appunto[]): TreeNote[] => {
        ...
        for (const noteId in noteMap) {
            // FIX: When iterating over object keys with for...in, TypeScript may not infer the value's type correctly. A type assertion is used to clarify that `noteMap[noteId]` is a `TreeNote`.
            const note = noteMap[noteId] as TreeNote;
            if (note.parentId && noteMap[note.parentId]) {
                (noteMap[note.parentId] as TreeNote).children.push(note);
            } else {
                tree.push(note);
            }
        }
    ...
    return (
        <div className={`w-full flex flex-col ${isEmbedded ? 'h-full' : 'max-w-7xl mx-auto h-[calc(100vh-100px)]'}`}>
            ...
            <div className="flex-grow flex gap-6 overflow-hidden">
                ...
                {!isEditorExpanded && (
                    <div className={`bg-gray-800 rounded-xl flex flex-col transition-all duration-300 ${isNoteListOpen ? (isEmbedded ? 'w-2/5' : 'w-2/5 p-4') : 'w-20 p-2 items-center'}`}>
                ...
                <div className="flex-grow p-6 overflow-y-auto">
                    {isProcessing ? (
                        ...
                        <div
                            ...
                            dangerouslySetInnerHTML={{ __html: renderMarkdownToHTML(result?.review || '') }}
                        >
                        </div>
                    )}
                </div>

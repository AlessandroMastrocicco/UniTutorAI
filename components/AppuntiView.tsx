...
export const AppuntiView: React.FC<AppuntiViewProps> = ({ subjectName, dataVersion, onSave, onDelete, onGetAiReview, isProcessingAI, t, isEmbedded = false }) => {
    ...
    useEffect(() => {
        const loadViewData = async () => {
            ...
            // FIX: Explicitly type `subjectNotes` as `Appunto[]` to prevent type inference issues where `notes` could be treated as `unknown[]` or `any[]`, causing errors when accessing properties like `parentId`.

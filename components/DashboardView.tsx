...
import type { DashboardData, DashboardEvent, FocusDispensa, GlobalTutorState } from '../types';
...
interface DashboardViewProps {
    ...
    tutorState: GlobalTutorState | null;
...
export const DashboardView: React.FC<DashboardViewProps> = ({ data, subjects, onSubjectAdd, onSubjectDelete, onSubjectRename, onNavigate, tutorState, activeSubject, onCompleteTask, isTutorSyncing }) => {
    ...
    const firstUncompletedIndex = tutorState?.tasks.findIndex(t => !t.isCompleted) ?? -1;
                ...
                {activeSubject && (
                    <div className="bg-gray-800 rounded-xl shadow-lg p-6">
                        <h2 className="text-xl font-bold text-white mb-4 flex items-center">
                            <TargetIcon />
                            <span className="ml-2">Obiettivi di Oggi per "{activeSubject}"</span>
                        </h2>
                        {tutorState?.tasks && tutorState.tasks.length > 0 ? (
                            <ul className="space-y-3">
                                {tutorState.tasks.map((task, index) => {
                            ...
                            <p className="text-gray-400 text-center py-8">
                                {isTutorSyncing ? "Sincronizzazione degli obiettivi in corso..." : "Nessun obiettivo per oggi. Sincronizza il tutor per generarne di nuovi."}
                            </p>
                        )}
                         {firstUncompletedIndex === -1 && tutorState && tutorState.tasks.length > 0 && (
                            <div className="text-center mt-4 p-4 bg-green-900/50 rounded-lg">
                                <p className="font-bold text-green-300">🎉 Complimenti! Hai completato tutti i task di oggi!</p>
                            </div>
                        )}
                    </div>
                )}

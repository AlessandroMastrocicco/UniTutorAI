...
export const DatabaseAppView: React.FC<DatabaseAppViewProps> = ({ selectedSubject, dataVersion }) => {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto flex flex-col h-[calc(100vh-100px)]">
            <header className="w-full mb-6 text-center flex-shrink-0">
                <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">Database Materia</h1>
                                        ...
                                        <div className="px-4 pb-4 border-t border-gray-700 mt-2 pt-3">
                                            <div className="space-y-3">
                                                {macro.pointers.map(p => {
                                                    ...
                                                    const totalAttempts = exerciseStats.appearances + simulationStats.appearances;
                                                    const totalSuccess = exerciseStats.completed + simulationStats.correct;
                                                    const mastery = totalAttempts > 0 ? (totalSuccess / totalAttempts) * 100 : 0;
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className="flex justify-between items-start gap-2">
                                                                ...
                                                                <span className={`flex-shrink-0 text-xs font-bold px-2 py-1 rounded-full ${p.affrontato ? 'bg-green-900 text-green-300' : 'bg-gray-600 text-gray-300'}`}>
                                                                    {p.affrontato ? 'Affrontato' : 'Non Affrontato'}
                                                                </span>
                                                            </div>
                                                            ...
                                                            <div className="mt-3 pt-3 border-t border-gray-600/50">
                                                                <div className="flex justify-between items-center mb-1">
                                                                    ...
                                                                    <span className="text-xs font-medium text-white">{mastery.toFixed(0)}%</span>
                                                                </div>
                                                                <div className="w-full bg-gray-600 rounded-full h-1.5">
                                                                    <div className="bg-indigo-500 h-1.5 rounded-full" style={{ width: `${mastery}%` }}></div>
                                                                </div>

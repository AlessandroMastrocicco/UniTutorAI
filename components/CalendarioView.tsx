...
const formatDateRange = (startDate: Date, t: (key: string) => string): string => {
    const start = new Date(startDate);
    const end = new Date(startDate);
    end.setDate(start.getDate() + 6);

    const languageCode = t('sidebar.dashboard') === 'Dashboard' ? 'en-US' : 'it-IT';

    const options: Intl.DateTimeFormatOptions = { month: 'long', day: 'numeric' };
    
    const startStr = start.toLocaleDateString(languageCode, options);
    const endStr = end.toLocaleDateString(languageCode, options);

    if (start.getFullYear() !== end.getFullYear()) {
         return `${start.toLocaleDateString(languageCode, {...options, year: 'numeric'})} - ${end.toLocaleDateString(languageCode, {...options, year: 'numeric'})}`;
    }
...
const START_HOUR = 8;
const END_HOUR = 20;
const daysOfWeekIT = ['Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato', 'Domenica'];
...
const NewEventModal: React.FC<{
    ...
    subjects: string[];
    t: (key: string) => string;
}> = ({ isOpen, onClose, onSave, subjects, t }) => {
...
const getNowInRome = (languageCode: 'it-IT' | 'en-US') => {
    const now = new Date();
    const formatter = new Intl.DateTimeFormat(languageCode, {
        timeZone: 'Europe/Rome',
        year: 'numeric', month: 'numeric', day: 'numeric',
        hour: 'numeric', minute: 'numeric', weekday: 'long', hourCycle: 'h23'
    });
...
export const CalendarioView: React.FC<CalendarioViewProps> = ({ subjects, allSubjectsData, onAddEvent, t }) => {
    const languageCode = useMemo(() => (t('sidebar.dashboard') === 'Dashboard' ? 'en-US' : 'it-IT'), [t]);
    const [nowInRome, setNowInRome] = useState(() => getNowInRome(languageCode));
    ...
    const daysOfWeek = t('sidebar.dashboard') === 'Dashboard' ? daysOfWeekEN : daysOfWeekIT;

    useEffect(() => {
        const timer = setInterval(() => setNowInRome(getNowInRome(languageCode)), 60000); // Update every minute
        setNowInRome(getNowInRome(languageCode)); // also update when language changes
    ...
    const currentTimePosition = useMemo(() => {
        ...
        return null;
    }, [isCurrentWeek, nowInRome, daysOfWeek]);
    ...
    return (
        <div className="w-full max-w-7xl mx-auto flex flex-col h-[calc(100vh-100px)]">
            ...
            <div className="flex items-center justify-between mb-4 p-4 bg-gray-900/50 rounded-lg">
                <h2 className="text-lg font-semibold">{formatDateRange(weekStart, t)}</h2>
                {subjects.length > 0 && (

...
const Switch: React.FC<{ checked: boolean; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; }> = ({ checked, onChange }) => {
    return (
...
interface ImpostazioniViewProps {
    ...
    userName: string;
    onUpdateTutorSettings: (settings: { enabled?: boolean; name?: string }) => void;
    t: (key: string) => string;
}

export const ImpostazioniView: React.FC<ImpostazioniViewProps> = ({ theme, onThemeChange, language, onLanguageChange, pointerVisibility, onPointerVisibilityChange, isGlobalTutorEnabled, userName, onUpdateTutorSettings, t }) => {
    const [activeTab, setActiveTab] = useState<'appearance' | 'logic' | 'tutor'>('appearance');
                ...
                {activeTab === 'tutor' && (
                    <div className="space-y-4">
                        ...
                        <div className="p-4 bg-gray-900/50 rounded-lg">
                             <div>
                                <h3 className="font-semibold text-white">{t('settings.userName')}</h3>
                                <p className="text-sm text-gray-400 mb-2">{t('settings.userNameDescription')}</p>
                            </div>
                            <input
                                type="text"
                                value={userName}

...
const EnlargeWidthIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 12h16m-7-7l7 7-7 7" /></svg>;
...
export const SolutionEditor: React.FC<{
    ...
    const [editorWidth, setEditorWidth] = useState(800); // New state for width
    ...
    const toggleMode = (newMode: 'text' | 'draw' | 'erase') => {
        // When switching from a drawing mode to text mode, commit the drawing to an image
        if ((mode === 'draw' || mode === 'erase') && newMode === 'text') {
            const canvas = canvasRef.current;
            if (canvas) {
                // Create a blank canvas to check if the current one has any content
                const blank = document.createElement('canvas');
                blank.width = canvas.width;
                blank.height = canvas.height;
                // Only proceed if the canvas is not blank
                if (canvas.toDataURL() !== blank.toDataURL()) {
                    ...
                    img.style.backgroundColor = 'white'; // Ensures visibility in dark mode
                    ...
                    if (editor) {
                        ...
                        if (selection && selection.rangeCount > 0) {
                            ...
                            // Add a non-breaking space after the image to place the cursor
                            const space = document.createTextNode('\u00A0');
                            fragment.appendChild(space);
                            
                            range.insertNode(fragment);

                            // Move cursor after the inserted image and space
                    ...
                    // Clear the drawing canvas now that it's embedded as an image
                    const ctx = canvas.getContext('2d');
                    ctx?.clearRect(0, 0, canvas.width, canvas.height);
                }
            }
            // Save the new state with the image in innerHTML and cleared canvas
            saveContentImmediate(); 
        }
        setMode(newMode);
    };
    ...
    return (
        <div className="flex-grow flex flex-col h-full">
            <div className="flex-shrink-0 flex flex-wrap items-center gap-1 sm:gap-2 p-2 bg-gray-900/50 border-b border-gray-700">
                ...
                <div className="border-l h-6 border-gray-600 mx-1"></div>
                <button onClick={() => setEditorWidth(w => w * 1.25)} className="p-1.5 hover:bg-gray-700 rounded" title="Aumenta larghezza area di scrittura"><EnlargeWidthIcon /></button>
                <button onClick={() => setEditorMinHeight(h => h * 1.5)} className="p-1.5 hover:bg-gray-700 rounded" title="Aumenta altezza area di scrittura"><EnlargeIcon /></button>
            ...
            <div ref={editorWrapperRef} className="relative flex-grow bg-gray-700/50 rounded-b-md overflow-y-auto overflow-x-auto">
                <div
                    ref={editorRef}
                    ...
                    style={{
                        ...
                        caretColor: mode === 'text' ? 'white' : 'transparent'
                    }}
                />
                <canvas
                    ref={canvasRef}

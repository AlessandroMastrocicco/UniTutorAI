...
const BookUploadIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>;
...
interface DispenseViewProps {
    ...
    subjectName: string;
    dataVersion: number;
    allNotes: Record<string, Appunto[]>;
...
const Resizer: React.FC<{ onMouseDown: (e: React.MouseEvent) => void }> = ({ onMouseDown }) => (
    <div
        ...
        className="hidden lg:block w-1.5 flex-shrink-0 bg-gray-700 hover:bg-indigo-500 active:bg-indigo-600 transition-colors cursor-col-resize z-20"
    />
);
...
export const DispenseView: React.FC<DispenseViewProps> = ({ subjectName, dataVersion, allNotes, onSave, onDelete, onUpdateStudiedPages, isProcessing, onAskTutorQuestion, onNavigateToNote, t }) => {
    ...
    // Split view state
    const [isNotesSplitViewOpen, setIsNotesSplitViewOpen] = useState(false);
    const [isLinkedNotesPopoverOpen, setIsLinkedNotesPopoverOpen] = useState(false);

    const viewerContainerRef = useRef<HTMLDivElement>(null);
    const [chatWidth, setChatWidth] = useState(400);
    const [notesWidth, setNotesWidth] = useState(400);

    const handleMouseDownChatResizer = useCallback((e: React.MouseEvent) => {
        e.preventDefault();
        ...
        const handleMouseMove = (moveEvent: MouseEvent) => {
            ...
            const containerWidth = viewerContainerRef.current?.offsetWidth || window.innerWidth;
            const otherPanelWidth = isNotesSplitViewOpen ? notesWidth : 0;
            
            const minWidth = 300;
            // The viewer panel must have at least 300px
            const maxWidth = containerWidth - otherPanelWidth - 300; 
    ...
    }, [chatWidth, isNotesSplitViewOpen, notesWidth]);

    const handleMouseDownNotesResizer = useCallback((e: React.MouseEvent) => {
        e.preventDefault();
        const startX = e.clientX;
        ...
        const handleMouseMove = (moveEvent: MouseEvent) => {
            const newWidth = startWidth - (moveEvent.clientX - startX);
            const containerWidth = viewerContainerRef.current?.offsetWidth || window.innerWidth;
            const otherPanelWidth = isChatOpen ? chatWidth : 0;

            const minWidth = 300;
            const maxWidth = containerWidth - otherPanelWidth - 300;
            
            setNotesWidth(Math.max(minWidth, Math.min(newWidth, maxWidth)));
        };
        ...
        document.body.style.cursor = 'col-resize';
        document.body.style.userSelect = 'none';
    }, [notesWidth, isChatOpen, chatWidth]);
    ...
    const linkedNotes = useMemo(() => {
        if (!viewingDispensa) return [];
        const subjectNotes = allNotes[subjectName] || [];
        const currentPageNumberInDispensa = viewingDispensa.pages[currentViewerPage - 1]?.pageNumber;
        if (!currentPageNumberInDispensa) return [];

        return subjectNotes.filter(note => 
            note.linkedDispensaId === viewingDispensa.id &&
            note.linkedPageNumbers?.includes(currentPageNumberInDispensa)
        );
    }, [currentViewerPage, viewingDispensa, allNotes, subjectName]);
    ...
    useEffect(() => {
        ...
        const loadPdfAndGenerateThumbnails = async () => {
            ...
            try {
                ...
                // FIX: The caught error `err` is of type 'unknown'. Checked if it is an instance of `Error` to safely access the `message` property, otherwise convert it to a string.
    ...
    return (
        <div className="w-full">
            ...
            <div className="w-full max-w-5xl mx-auto text-center mb-8 flex flex-wrap justify-center gap-4">
                 <button 
                    onClick={() => setIsModalOpen(true)}
                    disabled={isProcessing}
                    className="inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <PlusIcon />
                    {isProcessing ? 'Analisi in corso...' : t('dispense.uploadButton')}
                </button>
                <button 
                    onClick={() => alert('La funzionalità di caricamento libri è in fase di sviluppo e seguirà la strategia discussa!')}
                    disabled={isProcessing}
                    className="inline-flex items-center justify-center bg-sky-600 hover:bg-sky-700 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <BookUploadIcon />
                    {t('dispense.uploadBookButton')}
                </button>
            </div>
                ...
                <div className="fixed inset-0 bg-gray-900 z-50 flex flex-col">
                    <header className="flex-shrink-0 w-full bg-gray-800 p-3 flex justify-between items-center shadow-md">
                        ...
                        <div className="flex-1 min-w-0 text-center">
                            ...
                            <p className="text-sm text-gray-400">Pagina {currentViewerPage} di {viewingDispensa.pages.length}</p>
                        </div>
                        <div className="flex items-center gap-2 sm:gap-4">
                            <button 
                                onClick={() => setIsNotesSplitViewOpen(o => !o)}
                                className={`flex items-center gap-2 font-semibold py-2 px-4 rounded-lg transition-colors ${isNotesSplitViewOpen ? 'bg-indigo-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-white'}`}
                                title="Apri/Chiudi Appunti"
                            >
                                <AppuntiIcon />
                                <span className="hidden lg:block">Appunti</span>
                            </button>
                            <button 
                    ...
                    <main ref={viewerContainerRef} className="flex-grow flex flex-col lg:flex-row overflow-hidden">
                        {isChatOpen && (
                            <>
                                <div
                                    style={{ flexBasis: `${chatWidth}px` }}
                                    className="w-full flex-shrink-0 bg-gray-800/50 flex flex-col border-b lg:border-b-0 lg:border-r border-gray-700 h-1/2 lg:h-full"
                                >
                                ...
                                <Resizer onMouseDown={handleMouseDownChatResizer} />
                            </>
                        )}
                        
                        <div className="relative flex-grow flex items-center justify-center overflow-auto p-4 h-1/2 lg:h-full">
                            <button
                            ...
                            >
                             ...
                             {linkedNotes.length > 0 && (
                                <div className="absolute top-4 right-4 z-30">
                                    <button onClick={() => setIsLinkedNotesPopoverOpen(o => !o)} className="relative flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white rounded-full h-10 w-10 shadow-lg">
                                        <AppuntiIcon className="h-5 w-5" />
                                        <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">{linkedNotes.length}</span>
                                    </button>
                                    {isLinkedNotesPopoverOpen && (
                                        <div className="absolute top-12 right-0 bg-gray-800 border border-gray-700 rounded-lg shadow-2xl w-64 z-40 p-2">
                                            <h4 className="text-sm font-bold text-white p-2 border-b border-gray-700">Appunti Collegati</h4>
                                            <ul className="max-h-48 overflow-y-auto mt-2">
                                                {linkedNotes.map(note => (
                                                    <li key={note.id}>
                                                        <button onClick={() => { onNavigateToNote(note.id); setViewingDispensa(null); }} className="w-full text-left text-sm text-gray-300 p-2 hover:bg-gray-700 rounded-md">
                                                            {note.title}
                                                        </button>
                                                    </li>
                                                ))}
                         ...
                         {isNotesSplitViewOpen && (
                            <>
                                <Resizer onMouseDown={handleMouseDownNotesResizer} />
                                <div
                                    style={{ flexBasis: `${notesWidth}px` }}
                                    className="w-full flex-shrink-0 bg-gray-800/50 flex flex-col border-b lg:border-b-0 lg:border-l border-gray-700 h-1/2 lg:h-full"
                                >
                                    <AppuntiView
                                        isEmbedded
                                        ...
                                        onSave={() => {}}
                                        onDelete={() => {}}
                                        onGetAiReview={async () => ({ review: '', correctnessPercentage: 0, fidelityPercentage: null })}
                                        notes={allNotes[subjectName] || []}
                                        dispense={dispense}
                                        allThumbnails={allThumbnails}
                                        isProcessingAI={isProcessing}
                                        t={t}
                                        initialNoteToSelect={null}
                                        onNoteSelected={() => {}}
                                    />
                                </div>
                            </>

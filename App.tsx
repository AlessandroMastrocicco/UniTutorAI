...
const GlobalTutorView: React.FC<{
    ...
    t: (key: string) => string;
    sidebarOpen: boolean;
}> = ({ isOpen, onClose, state, userName, subjectName, onSendMessage, onDismissSuggestion, isProcessing, dataVersion, t, sidebarOpen }) => {
    ...
    const newSuggestions = state?.suggestions.filter(s => s.isNew) || [];
    const oldSuggestions = state?.suggestions.filter(s => !s.isNew) || [];
    ...
    return (
        <div className={`${baseClasses} ${layoutClasses}`} onClick={e => e.stopPropagation()}>
            ...
            <main className={`flex-grow flex overflow-hidden ${layout === 'full' ? 'flex-row' : 'flex-col'}`}>
                {/* Analysis Panel */}
                <div className={`flex-shrink-0 overflow-y-auto p-4 space-y-4 bg-gray-900/30 ${layout === 'full' ? 'w-1/2 border-r border-gray-700' : 'h-1/2 border-b border-gray-700'}`}>
                    <h3 className="text-md font-semibold text-white">Ciao, {userName || 'studente'}! Ecco la mia analisi per {subjectName}:</h3>
                    {state ? (
                        <>
                            <div className="bg-gray-700/50 p-3 rounded-lg"><p className="text-sm text-gray-300">{state.summary}</p></div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-green-900/50 p-3 rounded-lg">
                                    <h4 className="font-bold text-green-300 text-sm">Punti di Forza</h4>
                                    <ul className="list-disc list-inside text-xs text-green-200 mt-1 space-y-1">{state.strengths.map((s, i) => <li key={i}>{s}</li>)}</ul>
                                </div>
                                <div className="bg-yellow-900/50 p-3 rounded-lg">
                                    <h4 className="font-bold text-yellow-300 text-sm">Aree di Miglioramento</h4>
                                    <ul className="list-disc list-inside text-xs text-yellow-200 mt-1 space-y-1">{state.weaknesses.map((w, i) => <li key={i}>{w}</li>)}</ul>
                                </div>
                            </div>
                            {newSuggestions.length > 0 && <div>
                                <h4 className="font-semibold text-white mb-2">Suggerimenti per te:</h4>
                                {newSuggestions.map(s => (
                                    <div key={s.id} className="bg-indigo-900/50 p-3 rounded-lg relative">
                                        <p className="text-sm text-indigo-200 pr-6">{s.text}</p>
                                        <button onClick={() => onDismissSuggestion(s.id)} className="absolute top-1 right-1 p-1 text-indigo-200/70 hover:text-white" title="Nascondi suggerimento"><CloseIcon className="h-4 w-4" /></button>
                                    </div>
                                ))}
                            </div>}
                        </>
                    ) : (
                        <div className="flex items-center justify-center h-full text-gray-500 text-sm">Sincronizzazione in corso...</div>
                    )}
                </div>
                {/* Chat Panel */}
                <div className="flex-grow flex flex-col bg-gray-800/50">
                    ...
                    <div className="p-3 border-t border-gray-700">
                        <div className="flex items-center bg-gray-700 rounded-lg">
                            <input type="text" placeholder="Chiedi qualcosa al tutor..." value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} className="flex-1 bg-transparent p-2 text-white placeholder-gray-400 focus:outline-none" disabled={isProcessing} />
...
const App: React.FC = () => {
    ...
    const handleTutorActivation = useCallback(async () => {
        ...
        try {
            ...
            let prompt;
            let langInstruction = language === 'it' ? 'italiano' : 'inglese';
...
COMPITO:
Analizza i dati e rispondi ESCLUSIVAMENTE con un oggetto JSON in lingua ${langInstruction}.
- "summary": Un riassunto conciso (1-2 frasi) dello stato attuale dello studente.
...
- "suggestions": Un array di 2-3 oggetti suggerimento. Ogni suggerimento deve avere: "id" (stringa UUID), "text" (il testo del suggerimento), "type" ('suggestion', 'encouragement', o 'warning'), e "isNew" (impostato a true).`;
...
COMPITO:
Aggiorna la tua analisi e fornisci nuovi suggerimenti. Rispondi ESCLUSIVAMENTE con un oggetto JSON in lingua ${langInstruction} con la stessa struttura di prima (summary, strengths, weaknesses, suggestions), riflettendo i cambiamenti e le nuove attività.`;
            }
                    ...
                    suggestions: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                type: { type: Type.STRING, enum: ['encouragement', 'suggestion', 'warning'] },
                                isNew: { type: Type.BOOLEAN }
                            },
                            required: ["id", "text", "type", "isNew"]
                        }
                    }
                },
    ...
    }, [selectedSubject, ai, addNotification, updateNotification, generateContentWithModelSwitching, t, userName, language]);


    const handleUpdateTutorSettings = useCallback(async (settings: { enabled?: boolean; name?: string }) => {
        ...
        await db.appState.put({ key: 'globalTutorSettings', value: { enabled: newEnabled, name: newName } });

        if (newEnabled && !wasEnabled) {
            handleTutorActivation();
        }
    }, [isGlobalTutorEnabled, userName, handleTutorActivation]);
        ...
        const loadInitialData = async () => {
            ...
            try {
                ...
                if (tutorSettingsState) {
                    ...
                    await db.appState.put({ key: 'globalTutorSettings', value: { enabled: false, name: '' } });
                }
...
${contextText || "Nessun testo di riferimento fornito. Basa la tua analisi sulla correttezza generale dell'argomento e imposta 'fidelityPercentage' a null."}`;
    ...
    const handleDismissTutorSuggestion = useCallback(async (suggestionId: string) => {
        if (!selectedSubject || !globalTutorState) return;
    
        const newSuggestions = globalTutorState.suggestions.map(s =>
            s.id === suggestionId ? { ...s, isNew: false } : s
        );
    
        const newState: GlobalTutorState = { ...globalTutorState, suggestions: newSuggestions };
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-gray-200">
            ...
            <GlobalTutorView
                ...
                onDismissSuggestion={handleDismissTutorSuggestion}
                isProcessing={isProcessingAI}
                dataVersion={dataVersion}
                t={t}

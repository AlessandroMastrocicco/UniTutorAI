...
const GLOBAL_TUTOR_SYSTEM_INSTRUCTION = (userName: string, language: 'it' | 'en') => `
Sei UniTutorAI, un assistente allo studio universitario interdisciplinare e olistico. Il tuo obiettivo è assistere lo studente, di nome ${userName || 'studente'}, nel suo percorso di apprendimento globale, aiutandolo a creare connessioni tra le diverse materie.
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            try {
                ...
                const [subjectsDataState, tutorSettingsState, launchFlag, versionOneFlag, globalHistoryState] = await db.appState.bulkGet(['subjectsData', 'globalTutorSettings', 'hasLaunchedBefore', 'uniTutorAIVersion1WelcomeSeen', 'globalTutorHistory']);
...
${contextText || "Nessun testo di riferimento fornito. Basa la tua analisi sulla correttezza generale dell'argomento e imposta 'fidelityPercentage' a null."}`;
    ...
    const handleSendGlobalTutorMessage = useCallback(async (message: string) => {
        ...
        try {
            ...
            if (!globalChatSessionRef.current) {
                const systemInstruction = GLOBAL_TUTOR_SYSTEM_INSTRUCTION(userName, language);
    ...
    }, [ai, globalTutorChatHistory, selectedSubject, allEsercizioTopics, subjects, userName, language, generateContentWithModelSwitching, pendingTutorAction, subjectsData]);
...
Basandoti su tutto, fornisci una valutazione finale e un voto complessivo da 1 a 30 in ${langInstruction}. La parte orale dovrebbe pesare circa il 30-40% del voto finale. Rispondi ESCLUSIVAMENTE con un oggetto JSON con questa struttura: { "evaluation": "...", "final_grade": ... }`;
    ...
    const renderMainContent = () => {
        ...
        switch (activeView) {
            case 'dashboard': return <DashboardView data={dashboardData} subjects={subjects} onSubjectAdd={handleAddSubject} onSubjectDelete={handleDeleteSubject} onSubjectRename={handleRenameSubject} onNavigate={handleNavigate} tutorState={globalTutorState} activeSubject={selectedSubject} onCompleteTask={handleCompleteTutorTask} isTutorSyncing={isProcessingAI} />;
            ...
            default: return <DashboardView data={dashboardData} subjects={subjects} onSubjectAdd={handleAddSubject} onSubjectDelete={handleDeleteSubject} onSubjectRename={handleRenameSubject} onNavigate={handleNavigate} tutorState={globalTutorState} activeSubject={selectedSubject} onCompleteTask={handleCompleteTutorTask} isTutorSyncing={isProcessingAI} />;
        }
    }

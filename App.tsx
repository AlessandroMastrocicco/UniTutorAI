...
${contextText || "Nessun testo di riferimento fornito. Basa la tua analisi sulla correttezza generale dell'argomento e imposta 'fidelityPercentage' a null."}`;
        ...
        try {
            ...
            if (pendingTutorAction?.type === 'AWAITING_SOLUTION') {
                ...
                if (exerciseMatch) {
                    ...
                    let taskContext = '';
                    if (globalTutorState && globalTutorState.tasks.length > 0) {
                        const uncompletedTasks = globalTutorState.tasks.filter(t => !t.isCompleted).map(t => `- ${t.text}`).join('\n');
                        if (uncompletedTasks) {
                            const taskHeader = langInstruction === 'italiano' ? 'COMPITI ATTUALI DELLO STUDENTE' : "STUDENT'S CURRENT TASKS";
                            taskContext = `\n\n${taskHeader}:\n${uncompletedTasks}\n\n`;
                        }
                    }

                    const messageWithContext = contextForGeneration
                        ? `Basandoti su queste informazioni:\n---\n${contextForGeneration}\n---\n${taskContext}Rispondi alla seguente domanda: ${message}`
                        : `${taskContext}${message}`;
                    
                    const systemInstruction = `Sei un tutor AI per uno studente universitario di nome ${userName || 'studente'} per la materia "${selectedSubject}". Aiutalo con i suoi dubbi e tienilo concentrato sui suoi compiti attuali. Sii incoraggiante e conciso. Rispondi in ${langInstruction}.`;
    ...
    }, [selectedSubject, ai, generateContentWithModelSwitching, refreshData, language, pendingTutorAction, allEsercizioTopics, dispense, handleGenerateSimulation, globalTutorState, userName]);

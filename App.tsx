...
const OnboardingModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSave: (data: { name?: string; degreeCourse?: string }) => void;
    onLanguageSelect: (lang: 'it' | 'en') => void;
    t: (key: string) => string;
}> = ({ isOpen, onClose, onSave, onLanguageSelect, t }) => {
    const [step, setStep] = useState(1);
    const [nameInput, setNameInput] = useState('');
    const [degreeInput, setDegreeInput] = useState('');

    const handleLanguageChoice = (lang: 'it' | 'en') => {
    ...
    const handleDegreeContinue = () => {
        if (degreeInput.trim()) {
            onSave({ degreeCourse: degreeInput.trim() });
        }
        onClose();
    };
        ...
        switch (step) {
                ...
                return (
                    <>
                        <div className="p-8 text-center">
                            ...
                            <p className="text-gray-300 my-4 leading-relaxed">Select the language for the application and the AI Tutor.</p>
                            <div className="flex flex-col sm:flex-row gap-4 mt-6">
                                <button onClick={() => handleLanguageChoice('it')} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg transition-colors text-lg flex items-center justify-center gap-2">
                                    <span role="img" aria-label="Italian Flag">🇮🇹</span> Italiano
                                </button>
                                <button onClick={() => handleLanguageChoice('en')} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg transition-colors text-lg flex items-center justify-center gap-2">
            ...
            case 2: // Welcome
                return (
                    <>
                        <div className="p-8 text-center">
                            <SparkleIcon className="h-16 w-16 text-indigo-400 mx-auto mb-4" />
                            <h2 className="text-3xl font-bold text-white mb-4">{t('onboarding.welcomeTitle')}</h2>
                            <p className="text-gray-300 my-4 leading-relaxed">{t('onboarding.welcomeMessage')}</p>
                        </div>
                        <div className="bg-gray-900/50 px-6 py-4 flex justify-end gap-4 rounded-b-2xl">
                            <button onClick={() => setStep(3)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg transition-colors">{t('onboarding.continueButton')}</button>
                        </div>
                    </>
                );
            case 3: // Name
                 return (
                    <>
                        <div className="p-8">
                            <h2 className="text-2xl font-bold text-white mb-2 text-center">{t('onboarding.nameTitle')}</h2>
                            <p className="text-gray-400 mb-6 text-center">{t('onboarding.nameMessage')}</p>
                            <input
                                type="text"
                                value={nameInput}
                        ...
                        <div className="bg-gray-900/50 px-6 py-4 flex justify-end gap-4 rounded-b-2xl">
                            <button onClick={() => setStep(4)} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg">{t('onboarding.skipButton')}</button>
                            <button onClick={handleNameContinue} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg">{t('onboarding.continueButton')}</button>
                        </div>
                    </>
                );
            case 4: // Degree Course
                 return (
                    <>
                        <div className="p-8">
                            <h2 className="text-2xl font-bold text-white mb-2 text-center">{t('onboarding.degreeTitle')}</h2>
                            <p className="text-gray-400 mb-6 text-center">{t('onboarding.degreeMessage')}</p>
                            <input
                                type="text"
                                value={degreeInput}
                                onChange={(e) => setDegreeInput(e.target.value)}
                                placeholder={t('onboarding.degreePlaceholder')}
                                onKeyPress={e => e.key === 'Enter' && handleDegreeContinue()}
                                className="w-full bg-gray-700 border border-gray-600 rounded-lg p-3 text-white focus:ring-2 focus:ring-indigo-500"
                                autoFocus
                            />
                        </div>
                        <div className="bg-gray-900/50 px-6 py-4 flex justify-end gap-4 rounded-b-2xl">
                             <button onClick={onClose} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg">{t('onboarding.skipButton')}</button>
                             <button onClick={handleDegreeContinue} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg">{t('onboarding.continueButton')}</button>
                        </div>
                    </>
                );
            default: return null;
...
const QuickActionsGridIcon: React.FC<{ className?: string }> = ({ className }) => (
...
const GlobalTutorView: React.FC<{
    ...
    onCreateQuickExercise: (topicQuery: string) => Promise<void>;
    onCreateQuickSimulation: () => Promise<void>;
    onCompleteTask: (taskId: string, subjectName: string) => void;
    isProcessing: boolean;
    t: (key: string) => string;
    pendingAction: PendingTutorAction | null;
    onCancelAction: () => void;
}> = ({ isOpen, onClose, state, userName, activeSubject, allSubjects, chatHistory, onSendMessage, onCreateQuickExercise, onCreateQuickSimulation, onCompleteTask, isProcessing, t, pendingAction, onCancelAction }) => {
    const [chatInput, setChatInput] = useState('');
    const [activeTab, setActiveTab] = useState<'tasks' | 'chat'>('tasks');
    const [isQuickActionsOpen, setIsQuickActionsOpen] = useState(false);
    const [quickActionInput, setQuickActionInput] = useState<{ type: 'exercise_topic', value: string } | null>(null);

    const chatMessagesEndRef = useRef<HTMLDivElement>(null);

    // New states for position and size
    const [pos, setPos] = useState({ x: window.innerWidth - 450 - 24, y: window.innerHeight - (window.innerHeight * 0.7) - 96 });
    const [size, setSize] = useState({ width: 450, height: window.innerHeight * 0.7 });
    ...
    const onDragMouseDown = (e: React.MouseEvent) => {
        if (e.target !== e.currentTarget && (e.target as HTMLElement).closest('button')) return; // Ignore button clicks in header
        setIsDragging(true);
        dragStartRef.current = { startX: e.clientX, startY: e.clientY, initialX: pos.x, initialY: pos.y };
    };
    
    const onResizeMouseDown = (e: React.MouseEvent) => {
        e.stopPropagation();
        setIsResizing(true);
        resizeStartRef.current = { startX: e.clientX, startY: e.clientY, initialW: size.width, initialH: size.height };
    };
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            if (isDragging) {
                ...
                setPos({ x: dragStartRef.current.initialX + dx, y: dragStartRef.current.initialY + dy });
            }
            if (isResizing) {
                const dw = e.clientX - resizeStartRef.current.startX;
                const dh = e.clientY - resizeStartRef.current.startY;
                setSize({ width: Math.max(350, resizeStartRef.current.initialW + dw), height: Math.max(300, resizeStartRef.current.initialH + dh) });
            }
        };
        const handleMouseUp = () => {
            setIsDragging(false);
    ...
    const handleQuickAction = (action: 'analyze_subject' | 'daily_plan' | 'create_exercise' | 'create_simulation') => {
        setIsQuickActionsOpen(false);
        switch(action) {
            case 'analyze_subject':
                onSendMessage('QUICK_ACTION_ANALYZE_DATA');
                break;
            case 'daily_plan':
                onSendMessage('QUICK_ACTION_STUDY_PLAN');
                break;
            case 'create_exercise':
                setQuickActionInput({ type: 'exercise_topic', value: '' });
                break;
            case 'create_simulation':
                onCreateQuickSimulation();
                break;
        }
    };

    const handleQuickActionSubmit = () => {
        if (quickActionInput?.type === 'exercise_topic' && quickActionInput.value.trim()) {
            onCreateQuickExercise(quickActionInput.value.trim());
    ...
    return (
        <div
            ...
            className="bg-gray-800 shadow-2xl rounded-2xl flex flex-col border border-gray-700 z-40"
            onClick={e => e.stopPropagation()}
        >
            <header 
                onMouseDown={onDragMouseDown}
                style={{ cursor: 'move' }}
                className="flex-shrink-0 p-3 flex justify-between items-center border-b border-gray-700 bg-gray-900/50 rounded-t-2xl"
            >
            ...
            <main className="flex-grow flex flex-col overflow-hidden min-h-0">
                                                ...
                                                <li key={task.id} className={`flex items-start gap-3 p-3 rounded-lg transition-all ${isLocked ? 'bg-gray-900/50' : 'bg-gray-700/50'}`}>
                                                    <div className="pt-1 flex-shrink-0">
                                                        {isCompleted ? (
                                                            ...
                                                            <button onClick={() => onCompleteTask(task.id, activeSubject)} className="h-6 w-6 rounded-full border-2 border-indigo-400 bg-gray-800 hover:bg-indigo-500 transition-colors" title="Segna come completato"></button>
                                                        ) : (
                                                    ...
                                                    <p className={`text-sm ${isCompleted ? 'line-through text-gray-500' : isLocked ? 'text-gray-600' : 'text-gray-300'}`}>
                                                        {task.type === 'recovery' && (
                                                            <span className="inline-block mr-2 text-yellow-400 align-middle" title="Task di Recupero">
                ...
                {activeTab === 'chat' && (
                    <div className="flex-grow flex flex-col bg-gray-800/50 min-h-0">
                        ...
                        {quickActionInput?.type === 'exercise_topic' ? (
                             <div className="p-3 border-t border-gray-700 bg-indigo-900/50">
                                <p className="text-sm font-semibold text-indigo-300 mb-2">Per quale argomento vuoi creare un esercizio?</p>
                                <div className="flex items-center bg-gray-700 rounded-lg">
                                    <input type="text" placeholder="Es: Integrali definiti" value={quickActionInput.value} onChange={e => setQuickActionInput({ ...quickActionInput, value: e.target.value })} onKeyPress={e => e.key === 'Enter' && handleQuickActionSubmit()} className="flex-1 bg-transparent p-2 text-white placeholder-gray-400 focus:outline-none" autoFocus />
                                    <button onClick={handleQuickActionSubmit} className="p-2 text-white transition-colors disabled:text-gray-500" disabled={!quickActionInput.value.trim()}>Crea</button>
                                    <button onClick={() => setQuickActionInput(null)} className="p-2 text-gray-400 hover:text-white"><CloseIcon /></button>
                                </div>
                            </div>
                        ) : (
                            <div className="p-3 border-t border-gray-700 relative">
                                {isQuickActionsOpen && (
                                    <div className="absolute bottom-full left-3 right-3 mb-2 bg-gray-900 border border-gray-700 rounded-xl shadow-lg p-2 space-y-1 z-10">
                                        <button onClick={() => handleQuickAction('analyze_subject')} disabled={!activeSubject} className="w-full text-left text-sm p-2 rounded-md hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">Analizza dati materia</button>
                                        <button onClick={() => handleQuickAction('daily_plan')} className="w-full text-left text-sm p-2 rounded-md hover:bg-gray-700">Crea piano di studio per la giornata</button>
                                        <button onClick={() => handleQuickAction('create_exercise')} disabled={!activeSubject} className="w-full text-left text-sm p-2 rounded-md hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">Crea esercizio</button>
                                        <button onClick={() => handleQuickAction('create_simulation')} disabled={!activeSubject} className="w-full text-left text-sm p-2 rounded-md hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">Crea simulazione</button>
                                    </div>
                                )}
                                <div className="flex items-center bg-gray-700 rounded-lg">
                                     <button onClick={() => setIsQuickActionsOpen(o => !o)} className="p-2 text-gray-300 hover:text-white transition-colors" title="Azioni rapide"><QuickActionsGridIcon className="h-5 w-5" /></button>
            ...
            <div 
                ...
                title="Resize"
            >
                <svg className="w-full h-full text-gray-600 rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M4 20l16-16m-4 0h4v4" />
                </svg>
            </div>
        </div>
    );
...
const GLOBAL_TUTOR_SYSTEM_INSTRUCTION = (userName: string, degreeCourse: string, language: 'it' | 'en') => `
Sei UniTutorAI, un assistente allo studio universitario interdisciplinare e olistico. Il tuo obiettivo è assistere lo studente, di nome ${userName || 'studente'} che frequenta il corso di laurea in "${degreeCourse || 'studi universitari'}", nel suo percorso di apprendimento globale, aiutandolo a creare connessioni tra le diverse materie.

**PERSONALITÀ:**
- **Incoraggiante e Olistico:** Mantieni un tono di supporto, cercando sempre di collegare le conoscenze.
- **Strategico e Conciso:** Fornisci risposte chiare, dirette e focalizzate.
- **Accademico ma Accessibile:** Usa un linguaggio preciso ma facile da capire.

**LOGICA DI CONTESTO:**
Nel prompt dell'utente, riceverai un 'Contesto Primario' e un 'Contesto Secondario'.
1.  **Contesto Primario:** Contiene i "pointers" (micro-argomenti) della materia su cui lo studente è attualmente focalizzato. **Questa è la tua fonte di verità principale.** Le tue risposte devono basarsi PRIMARIAMENTE su questo.
2.  **Contesto Secondario:** Contiene pointers potenzialmente pertinenti da ALTRE materie che lo studente sta seguendo.

**REGOLE FONDAMENTALI:**
- **Lingua:** Rispondi SEMPRE in ${language === 'it' ? 'italiano' : 'inglese'}.
- **Formule Matematiche:** Usa SEMPRE la sintassi LaTeX (delimitata da $...$ per inline e $$...$$ per display/blocco).
- **Gerarchia del Contesto:**
    - Dai **massima priorità** al Contesto Primario.
    - Usa il Contesto Secondario **SOLO SE è strettamente rilevante** per la domanda, per:
        a) Creare collegamenti interdisciplinari (es. "Interessante, questo concetto di derivata che stai studiando in Economia è definito formalmente in Analisi Matematica in questo modo...").
        b) Fornire una prospettiva più ampia o un'analogia utile.
    - Se usi il Contesto Secondario, **dichiaralo sempre** (es. "Facendo un collegamento con [Nome Altra Materia]...").
- **Se non sai, dillo:** Se nessuno dei contesti forniti contiene informazioni per rispondere, dichiara di non avere dati sufficienti ma prova a dare una risposta generale, se possibile.
- **Funzioni Speciali:**
  - Se ti viene chiesto di **"creare un esercizio su [argomento]"**, rispondi solo con la conferma che lo stai facendo.
  - Se ti viene chiesto di **"creare una simulazione"**, rispondi solo con la conferma.
`;
...
const App: React.FC = () => {
    ...
    const globalChatSessionRef = useRef<Chat | null>(null);
    ...
    const [isGlobalTutorEnabled, setIsGlobalTutorEnabled] = useState<boolean>(true);
    ...
    const [globalTutorChatHistory, setGlobalTutorChatHistory] = useState<ChatMessage[]>([]);
    ...
    useEffect(() => {
        const initializeAi = async () => {
            ...
            // Check if running in Electron and use the exposed API
            if (window.electronAPI) {
                try {
                    apiKey = await window.electronAPI.getApiKey();
                } catch (e) {
                    console.error("Failed to get API key via Electron IPC:", e);
                }
            } else {
                // Fallback for web environment (Vite dev server)
    ...
    const handleTutorActivation = useCallback(async () => {
        ...
        try {
            ...
            // 2. Aggregate weakness data for recovery tasks
            const weaknessData: string[] = [];
            const lowMasteryTopics = subjectTopics.filter(t => t.masteryScore !== undefined && t.masteryScore < 50).sort((a, b) => (a.masteryScore || 50) - (b.masteryScore || 50)).slice(0, 3);
            if (lowMasteryTopics.length > 0) weaknessData.push(`Argomenti con bassa padronanza: ${lowMasteryTopics.map(t => `"${t.title}" (Padronanza: ${t.masteryScore}%)`).join(', ')}`);

            const recentIncorrectInstances = subjectInstances.filter(i => i.status === 'corrected' && i.aiCorrectness === 'incorrect' && new Date(i.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).slice(-3);
            if (recentIncorrectInstances.length > 0) {
                const topicIds = new Set(recentIncorrectInstances.map(i => i.topicId));
                const relatedTopics = subjectTopics.filter(t => topicIds.has(t.id));
                weaknessData.push(`Esercizi recenti sbagliati sugli argomenti: ${[...new Set(relatedTopics.map(t => `"${t.title}"`))].join(', ')}`);
            }

            const recentSims = subjectSimulations.filter(s => new Date(s.date) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).slice(-2);
            recentSims.forEach(sim => {
                if ((sim.finalGrade ?? sim.grade) < 18) weaknessData.push(`Simulazione del ${new Date(sim.date).toLocaleDateString()} con voto basso (${(sim.finalGrade ?? sim.grade).toFixed(1)}/30).`);
            });

            let weaknessInfo = '';
            if (weaknessData.length > 0) {
                weaknessInfo = `\nDATI SULLE DEBOLEZZE RECENTI (da usare per task di recupero):\n- ${weaknessData.join('\n- ')}`;
            }

            // 3. Prepare contextual info conditionally
            let contextualInfo = '';
            if (isTutorContextualAnalysisEnabled) {
                ...
                const syllabus = subjectDetails.syllabus || 'Non fornito.';
                contextualInfo = `
...
- Programma del corso (approssimativo): ${syllabus}`;
            }
            ...
            const recoveryInstruction = language === 'it' 
                ? `Se identifichi una lacuna specifica basandoti sui "DATI SULLE DEBOLEZZE RECENTI", crea un task con type='recovery' per affrontarla. Questo task dovrebbe suggerire di ripassare un argomento specifico o di rifare un tipo di esercizio.`
                : `If you identify a specific gap based on the "RECENT WEAKNESS DATA", create a task with type='recovery' to address it. This task should suggest reviewing a specific topic or redoing a type of exercise.`;
...
COMPITO:
Analizza tutti i dati (contesto, debolezze, progressi) e rispondi ESCLUSIVAMENTE con un oggetto JSON in lingua ${langInstruction}.
${langTaskInstruction}
...
- "tasks": Un array di 2-4 oggetti task per un piano di studio giornaliero. Ogni task deve avere: "id" (stringa UUID), "text" (il testo del task), "isCompleted" (impostato SEMPRE a false), e "type" (una stringa tra 'study', 'review', 'practice'). ${recoveryInstruction}`;
                ...
                if (deltaParts.length === 0 && weaknessData.length === 0) {
...
COMPITO:
Aggiorna la tua analisi e fornisci un nuovo piano di studio giornaliero basandoti sulle nuove attività e sulle debolezze identificate. Rispondi ESCLUSIVAMENTE con un oggetto JSON in lingua ${langInstruction} con la struttura (summary, strengths, weaknesses, tasks). Ogni task deve avere id, text, isCompleted, e type. ${recoveryInstruction} ${langTaskInstruction}`;
            }
                    ...
                    tasks: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                type: { type: Type.STRING, enum: ['study', 'review', 'practice', 'recovery'] },
                            },
                            required: ["id", "text", "isCompleted", "type"]
                        }
                    }
                },
    ...
    const handleCompleteTutorTask = useCallback(async (taskId: string, subjectName: string) => {
        const tutorState = await db.appState.get(`globalTutorState_${subjectName}`);
        if (!tutorState?.value) return;
    
        const currentTutorState: GlobalTutorState = tutorState.value;
    
        const newTasks = currentTutorState.tasks.map(t =>
            t.id === taskId ? { ...t, isCompleted: true } : t
        );
    
        const newState: GlobalTutorState = { ...currentTutorState, tasks: newTasks };
    
        await db.appState.put({ key: `globalTutorState_${subjectName}`, value: newState });
    ...
    const handleUpdateTutorSettings = useCallback(async (settings: { enabled?: boolean; name?: string; degreeCourse?: string; contextualAnalysisEnabled?: boolean }) => {
        ...
        const newDegreeCourse = settings.degreeCourse !== undefined ? settings.degreeCourse : degreeCourse;
        ...
        setUserName(newName);
        setDegreeCourse(newDegreeCourse);
        setIsTutorContextualAnalysisEnabled(newContextual);

        await db.appState.put({ key: 'globalTutorSettings', value: { enabled: newEnabled, name: newName, degreeCourse: newDegreeCourse, contextualAnalysisEnabled: newContextual } });
    ...
    }, [isGlobalTutorEnabled, userName, degreeCourse, isTutorContextualAnalysisEnabled, handleTutorActivation]);

    const handleResetGlobalTutorChat = useCallback(async () => {
        setGlobalTutorChatHistory([]);
        await db.appState.put({ key: 'globalTutorHistory', value: [] });
        if (globalChatSessionRef.current) {
            globalChatSessionRef.current = null;
        }
        addNotification({ message: t('settings.tutorResetSuccess'), type: 'success' });
    }, [addNotification, t]);
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            try {
                ...
                const [subjectsDataState, tutorSettingsState, launchFlag, globalHistoryState] = await db.appState.bulkGet(['subjectsData', 'globalTutorSettings', 'hasLaunchedBefore', 'globalTutorHistory']);
                ...
                if (tutorSettingsState) {
                    setIsGlobalTutorEnabled(tutorSettingsState.value.enabled ?? true);
                    setUserName(tutorSettingsState.value.name || '');
                    setDegreeCourse(tutorSettingsState.value.degreeCourse || '');
                    setIsTutorContextualAnalysisEnabled(tutorSettingsState.value.contextualAnalysisEnabled ?? true);
                } else {
                    await db.appState.put({ key: 'globalTutorSettings', value: { enabled: true, name: '', degreeCourse: '', contextualAnalysisEnabled: true } });
                    setIsGlobalTutorEnabled(true);
                    setIsTutorContextualAnalysisEnabled(true);
                }
                
                if(globalHistoryState) {
                    setGlobalTutorChatHistory(globalHistoryState.value || []);
                }
...
4. Fornisci una stima del tempo di studio in minuti ("estimated_study_time") necessario per comprendere l'argomento, basandoti sulla sua densità e complessità.
...
${fullTextForAI}`;
                ...
                const microResponse = await generateContentWithModelSwitching({
                    ...
                    config: { systemInstruction: `Sei un AI che estrae micro-argomenti (pointers) e li classifica, ${langSystemInstruction}`, responseMimeType: "application/json", responseSchema: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { micro_topic: { type: Type.STRING }, page_numbers: { type: Type.ARRAY, items: { type: Type.INTEGER } }, question_types: { type: Type.ARRAY, items: { type: Type.STRING } }, estimated_study_time: { type: Type.INTEGER } }, required: ["micro_topic", "page_numbers", "question_types", "estimated_study_time"] } } },
                });
                const microTopicsRaw: { micro_topic: string; page_numbers: number[]; question_types: EsercizioQuestionType[]; estimated_study_time: number }[] = JSON.parse(microResponse.text.trim());
                ...
                allNewMicroTopics.push(...microTopicsRaw.map((item, index): EsercizioTopic | null => {
                    ...
                    return {
                        ...
                        estimatedStudyTime: item.estimated_study_time,
...
${contextText || "Nessun testo di riferimento fornito. Basa la tua analisi sulla correttezza generale dell'argomento e imposta 'fidelityPercentage' a null."}`;
    ...
    const handleSendGlobalTutorMessage = useCallback(async (message: string) => {
        ...
        const updatedHistory = [...globalTutorChatHistory, userMessage];
        setGlobalTutorChatHistory(updatedHistory);
        await db.appState.put({ key: 'globalTutorHistory', value: updatedHistory });
        
        setIsProcessingAI(true);
    
        try {
            // Special actions (like exercise generation) are now tied to the active subject context.
            if (selectedSubject && pendingTutorAction?.type === 'AWAITING_SOLUTION') {
                const { question, solution, topicId } = pendingTutorAction;
                setPendingTutorAction(null);

                const prompt = `Coreggi questo esercizio. DOMANDA: "${question}". SOLUZIONE UFFICIALE: "${solution}". RISPOSTA DELLO STUDENTE: "${message}". Fornisci un feedback costruttivo e indica se è corretto, parzialmente corretto, o sbagliato. Rispondi in ${language}.`;
                
                const response = await globalChatSessionRef.current?.sendMessage({ message: prompt });
                const modelMessage: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: response?.text || "Errore" };
                setGlobalTutorChatHistory(prev => [...prev, modelMessage]);
                await db.appState.put({ key: 'globalTutorHistory', value: [...updatedHistory, modelMessage] });
                setIsProcessingAI(false);
                return;
            }
            
            if (message === 'QUICK_ACTION_ANALYZE_DATA') {
                const subjectDetails = subjectsData[selectedSubject];
                if (!subjectDetails) {
                    throw new Error("Dettagli materia non trovati.");
                }
                const context = `Dati materia "${selectedSubject}":\nNome Docente: ${subjectDetails.professors?.[0]?.name || 'N/D'}\nOrari: ${subjectDetails.schedule.lessons.map(l => `${l.day} ${l.startTime}-${l.endTime}`).join(', ')}\nCFU: ${subjectDetails.cfu}\nProgramma: ${subjectDetails.syllabus}\nDate Esami: ${subjectDetails.examDates.map(d => d.date).join(', ')}`;
                const finalPrompt = `Analizza i seguenti dati per la materia e forniscimi un riassunto dei punti chiave e delle scadenze imminenti.\n\n${context}`;
                const response = await globalChatSessionRef.current?.sendMessage({ message: finalPrompt });
                const modelMessage: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: response?.text || "Errore" };
                setGlobalTutorChatHistory(prev => [...prev, modelMessage]);
                await db.appState.put({ key: 'globalTutorHistory', value: [...updatedHistory, modelMessage] });
                setIsProcessingAI(false);
            ...
            if (message === 'QUICK_ACTION_STUDY_PLAN') {
                let multiSubjectContext = '';
                for (const subject of subjects) {
                    const details = subjectsData[subject];
                    multiSubjectContext += `Materia: ${subject}, CFU: ${details.cfu}, Date Esami: ${details.examDates.map(d => d.date).join('; ') || 'Nessuna'}\n`;
                }
                const finalPrompt = `Basandoti sui dati delle mie materie, crea un piano di studio strategico per la giornata di oggi. Considera l'urgenza (date d'esame) e l'importanza (CFU).\n\n${multiSubjectContext}`;
                const response = await globalChatSessionRef.current?.sendMessage({ message: finalPrompt });
                const modelMessage: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: response?.text || "Errore" };
                setGlobalTutorChatHistory(prev => [...prev, modelMessage]);
                await db.appState.put({ key: 'globalTutorHistory', value: [...updatedHistory, modelMessage] });
            ...
            if (selectedSubject) {
                ...
                if (exerciseMatch) {
                    ...
                    if (bestTopic?.sourceContent) {
                        ...
                        const exerciseMessage: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: `Ecco un esercizio su "${bestTopic.title}" (materia: ${selectedSubject}):\n\n**Domanda:**\n${exercise.question}\n\nQuando vuoi, scrivimi la tua soluzione e la correggerò.` };
                        setGlobalTutorChatHistory(prev => [...prev, exerciseMessage]);
                        await db.appState.put({ key: 'globalTutorHistory', value: [...updatedHistory, exerciseMessage] });
            ...
            // Default to holistic, cross-subject chat
            if (!globalChatSessionRef.current) {
                const systemInstruction = GLOBAL_TUTOR_SYSTEM_INSTRUCTION(userName, degreeCourse, language);
                globalChatSessionRef.current = ai.chats.create({
                  model: 'gemini-2.5-flash',
                  history: globalTutorChatHistory,
                  config: { systemInstruction },
                });
            }
            ...
            if (selectedSubject && allTopics[selectedSubject]) {
                primaryContext = `--- CONTESTO PRIMARIO (Materia: ${selectedSubject}) ---\nElenco Pointers:\n${allTopics[selectedSubject].map(t => `- ${t.title} (Tempo stimato: ${t.estimatedStudyTime || '?'} min)`).join('\n')}\n--- FINE CONTESTO PRIMARIO ---`;
            }
    
            const otherSubjects = subjects.filter(s => s !== selectedSubject);
            if (otherSubjects.length > 0) {
                const secondaryPointers: { title: string; subject: string }[] = [];
                otherSubjects.forEach(sub => {
                    (allTopics[sub] || []).forEach(t => {
                        if (t.title.toLowerCase().includes(message.toLowerCase())) {
                            secondaryPointers.push({ title: t.title, subject: sub });
                        }
                    });
                });
                if (secondaryPointers.length > 0) {
                     secondaryContext = `--- CONTESTO SECONDARIO (Pointers pertinenti da altre materie) ---\n${secondaryPointers.map(p => `- ${p.title} (da ${p.subject})`).join('\n')}\n--- FINE CONTESTO SECONDARIO ---`;
                }
            }
    
            const finalPrompt = `${primaryContext}\n\n${secondaryContext}\n\nDOMANDA: ${message}`;
            const response = await globalChatSessionRef.current.sendMessage({ message: finalPrompt });
            ...
            setGlobalTutorChatHistory(prev => [...prev, errorMessage]);
            await db.appState.put({ key: 'globalTutorHistory', value: [...updatedHistory, errorMessage] });
        } finally {
            setIsProcessingAI(false);
        }
    }, [ai, globalTutorChatHistory, selectedSubject, allEsercizioTopics, subjects, userName, degreeCourse, language, generateContentWithModelSwitching, pendingTutorAction, subjectsData]);
...
Basandoti su tutto, fornisci una valutazione finale e un voto complessivo da 1 a 30 in ${langInstruction}. La parte orale dovrebbe pesare circa il 30-40% del voto finale. Rispondi ESCLUSIVAMENTE con un oggetto JSON con questa struttura: { "evaluation": "...", "final_grade": ... }`;
    ...
    const handleCreateQuickExercise = useCallback(async (topicQuery: string) => {
        if (!selectedSubject) {
            addNotification({ message: 'Seleziona una materia prima di creare un esercizio.', type: 'error' });
            return;
        }
        
        const subjectTopics = allEsercizioTopics[selectedSubject] || [];
        const bestTopic = subjectTopics.find(t => t.title.toLowerCase().includes(topicQuery.toLowerCase()));

        if (!bestTopic) {
            addNotification({ message: `Nessun argomento trovato per "${topicQuery}". Prova con un altro nome.`, type: 'error' });
            return;
        }

        const newInstance = await handleCreateEsercizio(bestTopic);
        
        if (newInstance) {
            addNotification({ message: `Esercizio su "${bestTopic.title}" creato! Lo trovi nella sezione Esercitazioni.`, type: 'success', duration: 5000 });
        }
    }, [selectedSubject, allEsercizioTopics, handleCreateEsercizio, addNotification]);

    const handleCreateQuickSimulation = useCallback(async () => {
        if (!selectedSubject) {
            addNotification({ message: 'Seleziona una materia prima di creare una simulazione.', type: 'error' });
            return;
        }
        ...
        if (subjectDispense.length === 0) {
            addNotification({ message: `Nessuna dispensa trovata per ${selectedSubject} su cui basare la simulazione.`, type: 'error' });
            return;
        }
        ...
        const newSimulation = await handleGenerateSimulation(dispensaIds, structure);

        if (newSimulation) {
            const simsState = await db.appState.get('allSimulations') || { key: 'allSimulations', value: {} };
            const allSimulationsData = simsState.value;
            const subjectSims = allSimulationsData[selectedSubject] || [];
            allSimulationsData[selectedSubject] = [...subjectSims, newSimulation];
            await db.appState.put({ key: 'allSimulations', value: allSimulationsData });
            
            addNotification({ message: `Simulazione per "${selectedSubject}" creata! La trovi nella sezione Simulazioni.`, type: 'success', duration: 5000 });
            refreshData();
        }
    }, [selectedSubject, handleGenerateSimulation, addNotification, refreshData]);
    ...
    const renderMainContent = () => {
        ...
        switch (activeView) {
            case 'dashboard': return <DashboardView data={dashboardData} subjects={subjects} onSubjectAdd={handleAddSubject} onSubjectDelete={handleDeleteSubject} onSubjectRename={handleRenameSubject} onNavigate={handleNavigate} onCompleteTask={handleCompleteTutorTask} isTutorSyncing={isProcessingAI} dataVersion={dataVersion} />;
            ...
            case 'impostazioni': return <ImpostazioniView 
                                            ...
                                            onResetTutorChat={handleResetGlobalTutorChat}
                                            t={t} 
                                        />;
            ...
            default: return <DashboardView data={dashboardData} subjects={subjects} onSubjectAdd={handleAddSubject} onSubjectDelete={handleDeleteSubject} onSubjectRename={handleRenameSubject} onNavigate={handleNavigate} onCompleteTask={handleCompleteTutorTask} isTutorSyncing={isProcessingAI} dataVersion={dataVersion} />;
        }
    }

    return (
        <div className="min-h-screen bg-gray-900 text-gray-200">
            <OnboardingModal isOpen={isFirstLaunch} onClose={() => setIsFirstLaunch(false)} onSave={handleUpdateTutorSettings} onLanguageSelect={handleLanguageChange} t={t} />
            ...
            <GlobalTutorView
                ...
                chatHistory={globalTutorChatHistory}
                onSendMessage={handleSendGlobalTutorMessage}
                onCreateQuickExercise={handleCreateQuickExercise}
                onCreateQuickSimulation={handleCreateQuickSimulation}
...
async function summarizeStudentProgress(
    ...
    const summaryParts: string[] = ["RIEPILOGO STATISTICHE:"];
    ...
    if (totalDispense > 0) {
        ...
        summaryParts.push(`- ${totalDispense} dispense caricate, livello di comprensione medio: ${(avgComprehension * 100).toFixed(0)}%.`);
    } else {
        summaryParts.push("- Nessuna dispensa caricata.");
    }
    ...
    if (instances.length > 0) {
        ...
        summaryParts.push(`- ${instances.length} esercizi generati, tasso di successo: ${successRate.toFixed(0)}%.`);
    } else {
        summaryParts.push("- Nessun esercizio svolto.");
    }
    ...
    if (simulations.length > 0) {
        ...
        if (completed.length > 0) {
            ...
            summaryParts.push(`- ${completed.length} simulazioni completate, voto medio: ${avgGrade.toFixed(1)}/30.`);
        }
    } else {
        summaryParts.push("- Nessuna simulazione completata.");
    }

    // Topics Summary (now more detailed)
    summaryParts.push(`\nDETTAGLIO ARGOMENTI (POINTERS):`);
    if (topics.length > 0) {
         topics.forEach(t => {
            const status = t.affrontato ? 'Affrontato' : 'Non affrontato';
            const time = t.estimatedStudyTime ? `${t.estimatedStudyTime} min` : 'N/D';
            summaryParts.push(`- Argomento: "${t.title}". Stato: ${status}. Tempo studio stimato: ${time}. Padronanza: ${t.masteryScore ?? 'N/D'}%.`);
        });
    } else {
        summaryParts.push('Nessun argomento (pointer) definito per questa materia.');
    }

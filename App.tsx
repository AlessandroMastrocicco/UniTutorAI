...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const initializeAi = () => {
            // Access the API key securely via the preload script exposed on the window object.
            const apiKey = (window as any).electronAPI?.getApiKey();
    ...
    useEffect(() => {
        ...
        const checkForUpdates = async () => {
            // FIX: This comparison appears to be unintentional because the types have no overlap. This check is likely a remnant from a template and is no longer needed since the repository path is configured.
    ...
    const handleRenameSubject = useCallback(async (oldName: string, newName: string) => {
        ...
        try {
            await db.transaction('rw', db.appState, db.dispense, async () => {
                await db.appState.put({ key: 'subjectsData', value: newSubjectsData });
...
${topicsContentForAI}`;
    ...
    const handleDeleteNote = useCallback(async (noteId: string) => {
        ...
        while(queue.length > 0) {
            const children = subjectNotes.filter((n: Appunto) => n.parentId === queue.shift());
    ...
    const renderMainContent = () => {
        ...
        switch (activeView) {
            ...
            case 'dispense': 
                ...
                return <DispenseView subjectName={selectedSubject} onSave={handleSaveDispensa} onDelete={handleDeleteDispensa} onUpdateStudiedPages={handleUpdateStudyProgress} onAskTutorQuestion={handleAskTutorQuestion} isProcessing={isProcessingAI} dataVersion={dataVersion} t={t} />;
            case 'appunti': 
